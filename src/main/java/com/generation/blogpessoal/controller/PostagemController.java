...
public class PostagemController { // Define a classe PostagemController
	...
	public ResponseEntity<Postagem> post(@Valid @RequestBody Postagem postagem) { // Define o método post que recebe uma Postagem no corpo da requisição e retorna uma Postagem dentro de um ResponseEntity
		...
		// equivalente ao INSERT INTO tb_postagens (data, texto, titulo) VALUES (current_timestamp(), 'Texto', 'Titulo');
	}
	...
	public ResponseEntity<Postagem> put(@Valid @RequestBody Postagem postagem) { //	 Define o método put que recebe uma Postagem no corpo da requisição e retorna uma Postagem dentro de um ResponseEntity
		return postagemRepository.findById(postagem.getId()) // Verifica se a postagem com o ID fornecido existe
				.map(resposta -> ResponseEntity.status(HttpStatus.OK).body(postagemRepository.save(postagem))) // Se existir, atualiza a postagem e retorna com status 200 (OK)
				.orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build()); // Se não existir, retorna status 404 (Not Found)

		// equivalente ao UPDATE tb_postagens SET data = current_timestamp(), texto = 'Texto', titulo = 'Titulo' WHERE id = ?;
	}
	...
	public void delete(@PathVariable Long id) { // Extrai o ID da URL e define o método delete que não retorna nada
        Optional<Postagem> postagem = postagemRepository.findById(id); // Ver
        if (postagem.isEmpty())
